#' Prepares the P data in particular and performs raster repojection to match
#' the core set.
#'
#' @param raw_data_directory the location of the file. This should be the in the
#'   same place as the other raw data.
#' @param forceUpdate ensures we don't accidentally run this re-projection again. It takes a long time!
...
prepare_p_layer <- function(raw_data_directory, forceUpdate){
  ...
  if(forceUpdate){
    raw_layers <- raster::raster(paste0(raw_data_directory, "/af250m_nutrient_p_t_m_agg30cm.tif"))
    ...
    raster::writeRaster(project_layers, file = paste(raw_data_directory, "phosphorous_layer_projected.tif", sep = "/"), format="GTiff", overwrite=TRUE)
  }
}
